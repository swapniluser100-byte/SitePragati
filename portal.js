...
function showCustomerDashboard(businessName) {
  ...
  document.getElementById("welcomeLine").textContent = businessName
...
async function loadLeads() {
  ...
  try {
    ...
    if (!data.leads || data.leads.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="8" class="empty-note">No leads yet.</td></tr>';
      return;
    }
    ...
    tbody.innerHTML =
      '<tr><td colspan="8" class="empty-note">Could not load leads.</td></tr>';
  }
}
...
async function loadCustomers() {
  ...
  try {
    ...
    list.querySelectorAll("[data-view-txns]").forEach((btn) => {
      btn.addEventListener("click", () =>
        openCustomerDetail(btn.dataset.viewTxns),
      );
    });
    list.querySelectorAll("[data-edit-cust]").forEach((btn) => {
      btn.addEventListener("click", () =>
        openCustomerForm(btn.dataset.editCust),
      );
    });
    list.querySelectorAll("[data-delete-cust]").forEach((btn) => {
      btn.addEventListener("click", () =>
        deleteCustomer(btn.dataset.deleteCust),
      );
    });
  } catch (err) {
...
async function loadTickets() {
  ...
  try {
    ...
    if (!data.tickets || data.tickets.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="5" class="empty-note">No tickets yet.</td></tr>';
      return;
    }
    ...
    tbody.querySelectorAll(".status-select").forEach((select) => {
      select.addEventListener("change", () =>
        updateTicketStatus(select.dataset.ticketId, select.value),
      );
    });
  } catch (err) {
    tbody.innerHTML =
...
async function loadTransactions(customerId) {
  ...
  try {
    ...
    if (!data.transactions || data.transactions.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="5" class="empty-note">No transactions yet.</td></tr>';
      return;
    }
    ...
    tbody.querySelectorAll("[data-edit-txn]").forEach((btn) => {
      btn.addEventListener("click", () =>
        openTransactionForm(
          data.transactions.find((t) => String(t.id) === btn.dataset.editTxn),
        ),
      );
    });
