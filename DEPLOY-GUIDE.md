...
```
This only creates the two new tables — it doesn't touch `case_studies` or `leads`. After this, the **Customers** tab in `/portal.html` (choose "Owner / Admin") will work.
...
**One page, two roles:** `/portal.html` is the single entry point for both you and your customers. At login, a toggle lets the person choose **"Customer"** or **"Owner / Admin"** — each leads to a completely separate dashboard with its own permissions, but there's only one URL to remember and share.

Visit `/portal.html`, select "Owner / Admin", and log in to manage leads and case studies — no more manually running SQL in the D1 Console for everyday updates.
...
3. Go to `https://yoursite.pages.dev/portal.html` and select **"Owner / Admin"**, log in with your `ADMIN_PASSWORD`
...
**Security notes:**
- `/portal.html` (choose "Owner / Admin") isn't linked from anywhere on the public site, but it's not secret either — anyone who guesses the URL sees the login screen (they can't get in without the password, but change your password if you ever suspect it leaked)
...
Lets your paying customers log in at `/portal.html` (choose "Customer") to view and raise maintenance tickets — separate from your admin login, and each customer only ever sees their own tickets.
...
3. **Give each customer a login:** go to `/portal.html` (choose "Owner / Admin") → **Customers** tab → **Edit** an existing customer (or **+ Add customer** for a new one) → fill in **Email** and **Password**. This becomes their login for `/portal.html` (choose "Customer"). Share these credentials with them yourself (WhatsApp, email, however you normally communicate) — there's no public sign-up.
...
**What you see:** a new **Tickets** tab in `/portal.html` (choose "Owner / Admin") showing tickets from *all* customers at once (with their business name), where you update status via a dropdown — same pattern as the Leads tab.
